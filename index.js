...
(function(global){
    ...
    for(let i = 0; i < numberOfParticles; i++) {
      if(Math.random()>=0) {
        switch(Math.floor(Math.random()*4)){
          case 0:
            particle = new Particle(0, Math.random()*h);
            break;
          case 1:
            particle = new Particle(Math.random()*w, 0);
            break;
          case 2:
            particle = new Particle(w, Math.random()*h);
            break;
          case 3:
            particle = new Particle(Math.random()*w, h);
        }
      }
      else {
  ...
  function drawBackground(alpha) {
    ctx.fillStyle = `rgba(135, 174, 115, ${alpha})`;
  ...
  function drawParticles() {
    ...
    ctx.strokeStyle = `hsla(${h}, ${colorConfig.colorSaturation}%, 20%, ${colorConfig.particleOpacity})`;
    let x;
    let y;
    particles.forEach(p => {
      x = p.pos.x / size;
